...
const contacts = [
    { icon: <Mail size={18} />,   label: "Email",        value: "rihab.jenzeri@gmail.com", href: "mailto:rihab.jenzeri@gmail.com" },
    { icon: <Phone size={18} />,  label: "Téléphone",    value: "+216 26 167 281",          href: "tel:+21626167281" },
    { icon: <MapPin size={18} />, label: "Localisation", value: "Tunis, Tunisie",            href: "#" },
];

const socials = [
    { icon: <Github size={20} />,   label: "GitHub",   href: "https://github.com/RihabJenzeri",                       color: "#e2e8f0" },
    { icon: <Linkedin size={20} />, label: "LinkedIn", href: "https://www.linkedin.com/in/rihab-jenzeri-42bb83202/",  color: "#0ea5e9" },
    { icon: <Twitter size={20} />,  label: "Twitter",  href: "#",                                                     color: "#38bdf8" },
];

const AVATAR_SRC = '/rihab.png';
...
const AvatarImg = ({ size }: { size: number }) => (
    <img
        ...
        onError={(e) => {
            ...
            parent.innerHTML = `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:white;font-size:${Math.round(size * 0.35)}px;font-weight:700">R</div>`;
        }}
    />
);

// ─────────────────────────────────────────────
//  PROFILE PANEL — rendu via Portal dans body
//  ✅ Échappe complètement le stacking context
//     du Header (backdrop-filter, z-index, etc.)
// ─────────────────────────────────────────────
const ProfilePanel = ({ onClose }: { onClose: () => void }) => {
    ...
    // ✅ Portal : le JSX est injecté directement dans document.body
    //    → aucun parent avec backdrop-filter ou z-index ne peut le piéger
    return ReactDOM.createPortal(
            ...
            <div style={{
                ...
                <div style={{ padding: '24px 28px', display: 'flex', gap: '18px', alignItems: 'center' }}>
                    <div style={{ position: 'relative', flexShrink: 0 }}>
                        ...
                        <div style={{ position: 'relative', width: '72px', height: '72px', borderRadius: '50%', overflow: 'hidden', zIndex: 1 }}>
                            <AvatarImg size={72} />
                        </div>
                        <div style={{
                    ...
                    {active === 'education' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            {education.map((edu, i) => (
                                ...
                                >
                                    <span style={{
                                        position: 'absolute', top: '14px', right: '14px', fontSize: '10px', color: '#06b6d4',
                                        background: 'rgba(6,182,212,0.1)', padding: '3px 8px', borderRadius: '20px',
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                                        ...
                                        <div style={{ flex: 1, paddingRight: '60px' }}>
                                            <div style={{ color: '#e2e8f0', fontSize: '14px', fontWeight: 700, lineHeight: 1.3 }}>{edu.degree}</div>
                                            <div style={{ color: '#94a3b8', fontSize: '12px', margin: '4px 0 6px' }}>{edu.school}</div>
                                            <div style={{ color: '#475569', fontSize: '11px', lineHeight: 1.4 }}>{edu.desc}</div>
                                        </div>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div>
                            ...
                            <div style={{
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                                    <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#22c55e', animation: 'profilePulse 2s infinite' }} />
                                    <span style={{ color: '#22c55e', fontSize: '13px', fontWeight: 700 }}>Disponible pour de nouveaux projets</span>
                                </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {contacts.map((c, i) => (
                                <a key={i} href={c.href} style={{
                                    ...
                                    border: '1px solid rgba(255,255,255,0.07)', textDecoration: 'none', transition: 'all 0.2s',
                                }}
                                ...
                                >
                                    ...
                                    <div style={{ flex: 1 }}>
                                        <div style={{ color: '#94a3b8', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '2px' }}>{c.label}</div>
                                        <div style={{ color: '#e2e8f0', fontSize: '13px', fontWeight: 600 }}>{c.value}</div>
                                    </div>
        ...
        document.body   // ✅ Portal : injecté directement dans <body>
    );
};
    ...
    return (
        <>
            ...
            >
                ...
                <div style={{ position: 'absolute', inset: '1px', borderRadius: '50%', background: '#0f172a' }} />
                <div style={{ position: 'relative', width: '36px', height: '36px', borderRadius: '50%', overflow: 'hidden', zIndex: 1 }}>
                    <AvatarImg size={36} />
                </div>
                <div style={{
            ...
            {/* ✅ Panel rendu via Portal → échappe le Header */}
    ...
    return (
        <div style={{
            ...
            <div style={{ textAlign: 'center' }}>
                <p style={{ color: '#475569', marginBottom: '16px', fontSize: '13px' }}>Cliquez sur l'icône pour ouvrir le profil</p>
                <ProfileTrigger />
            </div>
        </div>
    );
